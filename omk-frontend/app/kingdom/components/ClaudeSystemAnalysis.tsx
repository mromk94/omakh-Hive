...
export default function ClaudeSystemAnalysis() {
  const [analysisData, setAnalysisData] = useState<AnalysisData | null>(() => {
    // Load from localStorage on mount (preserve state when navigating)
  ...
  const fetchAnalysisData = async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      const url = `${BACKEND_URL}/api/v1/admin/claude/analysis${forceRefresh ? '?force_refresh=true' : ''}`;
      ...
      // Save to localStorage to preserve state when navigating
      localStorage.setItem('system_analysis_data', JSON.stringify(data));
      
      // Show cache info
      if (data.cached) {
        toast.success(`✅ Loaded from cache (${data.cache_age_hours}h old)`, { duration: 2000 });
      } else {
        toast.success('✅ Fresh analysis complete!', { duration: 2000 });
      }
    } catch (error: any) {
      ...
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-gray-400">
            {refreshing ? 'Analyzing Queen & Hive data...' : 'Running System Analysis...'}
          </p>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">
            Click below to run comprehensive system analysis.<br/>
            <span className="text-xs text-gray-500">Integrates codebase + Queen AI + Hive data</span>
          </p>
          <button
            onClick={() => fetchAnalysisData(false)}
            disabled={refreshing}
            className="px-6 py-3 bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-700 text-black font-semibold rounded-lg transition-all flex items-center gap-2 mx-auto"
          >
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-semibold">System Analysis</span>
            <div className="text-xs text-gray-500">
              {analysisData.cached ? (
                <span className="text-blue-400">
                  📦 Cached ({analysisData.cache_age_hours}h old) • {new Date(analysisData.timestamp).toLocaleString()}
                </span>
              ) : (
                <span className="text-green-400">
                  ✨ Fresh • {new Date(analysisData.timestamp).toLocaleString()}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => fetchAnalysisData(false)}
            disabled={refreshing}
            className="px-4 py-2 bg-gray-800 hover:bg-gray-700 disabled:bg-gray-900 text-white rounded-lg font-medium transition-all flex items-center gap-2 border border-gray-700"
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} />
            {refreshing ? 'Loading...' : 'Load Cached'}
          </button>
          <button
            onClick={() => fetchAnalysisData(true)}
            disabled={refreshing}
            className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 disabled:bg-gray-700 text-black font-semibold rounded-lg transition-all flex items-center gap-2"
          >
