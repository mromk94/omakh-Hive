...
async function main() {
  ...
  const artifactPath = path.join(__dirname, '..', 'artifacts', 'src', 'core', 'AdvisorsManager.sol', 'AdvisorsManager.json');
  const artifact = JSON.parse(await readFile(artifactPath, 'utf8'));
  const { abi, bytecode } = artifact;

  console.log('Deploying AdvisorsManager with wallet:', wallet.address);
  const factory = new ethers.ContractFactory(abi, bytecode, wallet);
  const contract = await factory.deploy(omk, admin, queen);
  console.log('TX:', contract.deploymentTransaction()?.hash);
  const receipt = await contract.deploymentTransaction().wait(2);
  const address = await contract.getAddress();
  console.log('AdvisorsManager deployed at:', address);
