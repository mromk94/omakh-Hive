...
async function main() {
  ...
  const artifactsDir = path.join(__dirname, '..', 'artifacts', 'src', 'core', 'BeeSpawner.sol');
  const artifactPath = path.join(artifactsDir, 'BeeSpawner.json');
  const artifact = JSON.parse(await readFile(artifactPath, 'utf8'));
  const { abi, bytecode } = artifact;

  console.log('Deploying BeeSpawner with wallet:', wallet.address);
