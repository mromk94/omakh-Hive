/**
 * Dynamic Contract Deployment Script (ESM)
...
async function main() {
  ...
  const constructorArgs = process.env.CONSTRUCTOR_ARGS 
    ...
    : [];
  if (!contractName) {
    throw new Error("CONTRACT_NAME env is required");
  }
  ...
  const deploymentsDir = path.join(path.dirname(new URL(import.meta.url).pathname), '..', 'deployments');
