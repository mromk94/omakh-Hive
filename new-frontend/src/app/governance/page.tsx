...
export default function GovernancePage() {
  ...
  const onOpenWidget = (key: string, label: string) => { setWKey(key); setWLabel(label); setOpen(true) }
    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="relative mx-auto max-w-3xl px-4 py-8">
        <div className="mb-5 flex items-center justify-between">
          <h1 className="text-glow text-xl font-semibold">🏛️ Governance</h1>
        ...
        <div className="float-bubble rounded-2xl border border-[#FFD700]/20 bg-[var(--panel)] p-4 sm:p-6">
          <WidgetRenderer widgetKey="governance" label="Governance" />
        </div>
      </div>
    </div>
  )
