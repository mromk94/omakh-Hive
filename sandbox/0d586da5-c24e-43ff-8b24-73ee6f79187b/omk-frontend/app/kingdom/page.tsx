...
export default function KingdomAdminPortal() {
  ...
  const loadDashboardStats = async () => {
    try {
      ...
      const hiveRes = await fetch('http://localhost:8001/api/v1/admin/hive/overview', { headers });
      if (hiveRes.ok) {
      ...
      const otcRes = await fetch('http://localhost:8001/api/v1/admin/otc/requests?status=pending', { headers });
      if (otcRes.ok) {
    ...
    const fetchData = async () => {
      try {
        ...
        const [analyticsRes, hiveRes] = await Promise.all([
          fetch('http://localhost:8001/api/v1/admin/analytics/overview', { 
            headers,
            signal: controller.signal 
          }),
          fetch('http://localhost:8001/api/v1/admin/hive/overview', { 
            headers,
            signal: controller.signal 
          }),
        ]);
...
function SystemConfigTab({ config, onUpdate }: any) {
  ...
  const handleSaveOTCPhase = async () => {
    setSaving(true);
    try {
      const response = await fetch('http://localhost:8001/api/v1/admin/config/otc-phase', {
        method: 'POST',
        headers: {
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={async () => {
              ...
              try {
                const response = await fetch('http://localhost:8001/api/v1/admin/config/treasury-wallets', {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
      ...
      <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
        ...
        <button
          onClick={async () => {
            ...
            try {
              const response = await fetch('http://localhost:8001/api/v1/admin/config/payment-methods', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
        ...
        <div className="space-y-4">
          ...
          <button
            onClick={async () => {
              ...
              try {
                const response = await fetch('http://localhost:8001/api/v1/admin/config/tge-date', {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
